...
def _delta(relation="works_at", *, a_type="person", b_type="org",
           a_email="dana@acme.org", origin="local", valid_to="",
           source_doc_id="msg-1"):
    return {
        "relations": [{"entity_a": "dana", "relation": relation, "entity_b": "acme",
...
def test_allowlisted_relation_contributes_edge_and_both_endpoints(tmp_path):
    ...
    recs = _outbox(s)
    assert n == 3                                   # dana entity, acme entity, works_at relation
...
def test_delta_since_watermark_picks_up_new_and_superseded(tmp_path):
    ...
    s = _store(tmp_path)
    with s._connect() as db:
        for eid, name, typ in (("dana", "Dana", "person"), ("acme", "Acme", "org"),
                               ("beta", "Beta", "org")):
            db.execute("INSERT INTO entities(id,name,type,origin) VALUES(?,?,?,'local')",
                       (eid, name, typ))
    graph_write.upsert_relation(s, "dana", "works_at", "acme", valid_from="2026-01-01",
                                source_doc_id="msg-1")
    delta, wm = org_contrib._delta_since_watermark(s)
    rels = {(r["entity_a"], r["relation"], r["entity_b"]) for r in delta["relations"]}
    assert ("dana", "works_at", "acme") in rels
    assert "dana" in delta["entities"] and "acme" in delta["entities"]
...
def test_ingest_is_idempotent(tmp_path):
    ...
    _write_batch(fs, "contrib/alice@x.org/1.jsonl",
                 [_rec({"kind": "entity", "id": "dana", "name": "Dana", "type": "person",
...
def test_materialise_writes_org_rows(tmp_path):
    ...
    _stage(s, [
        ...
        _rec({"kind": "entity", "id": "acme", "name": "Acme", "type": "org",
              "org": "", "email_addr": "", "aliases": ""}),
        _rec({"kind": "relation", "entity_a": "dana", "relation": "works_at", "entity_b": "acme"}),
    ])
...
def test_mentioned_with_singleton_stays_pending(tmp_path):
    s = _store(tmp_path)
    _stage(s, [
        _rec({"kind": "entity", "id": "dana", "name": "Dana", "type": "person", "org": "",
              "email_addr": "", "aliases": ""}),
        _rec({"kind": "entity", "id": "mary", "name": "Mary", "type": "person", "org": "",
              "email_addr": "", "aliases": ""}),
        _rec({"kind": "relation", "entity_a": "dana", "relation": "mentioned_with", "entity_b": "mary"}),
    ])
    res = org_curate._materialise(s)
...
def test_mentioned_with_two_sources_materialises(tmp_path):
    s = _store(tmp_path)
    ents = [_rec({"kind": "entity", "id": "dana", "name": "Dana", "type": "person", "org": "",
                  "email_addr": "", "aliases": ""}, sref="r1"),
            _rec({"kind": "entity", "id": "mary", "name": "Mary", "type": "person", "org": "",
                  "email_addr": "", "aliases": ""}, sref="r1")]
    rel = {"kind": "relation", "entity_a": "dana", "relation": "mentioned_with", "entity_b": "mary"}
...
def test_import_writes_org_rows(tmp_path):
    ...
    fs = LocalDirFleetStorage(tmp_path / "fleet")
    s = _store(tmp_path)
    _publish(fs, [_ent("dana", "Dana"), _ent("acme", "Acme", "org")],
             [{"entity_a": "dana", "relation": "works_at", "entity_b": "acme",
    ...
    with s._connect() as db:
        assert db.execute("SELECT COUNT(*) c FROM entities WHERE origin='org'").fetchone()["c"] == 2
        assert db.execute("SELECT origin FROM entity_relations WHERE entity_a='dana'").fetchone()["origin"] == "org"
...
def test_local_org_merge_org_survives(tmp_path, monkeypatch):
    ...
    with s._connect() as db:                          # local has MORE mentions, but org must win
        db.execute("INSERT INTO entities(id,name,type,email_addr,origin,mentions) "
                   "VALUES('dana-local','Dana','person','dana@x.org','local',9)")
        db.execute("INSERT INTO entities(id,name,type,email_addr,origin,mentions) "
                   "VALUES('dana-org','Dana','person','dana@x.org','org',1)")
    resolve.resolve_entities(s, home=str(tmp_path))
    assert s.get_entity("dana-org") is not None        # org survivor
    assert s.get_entity("dana-local") is None
...
def test_member_curator_consumer_round_trip(tmp_path, monkeypatch):
    ...
    dana = bob.store.get_entity("dana")
    assert dana is not None and dana["origin"] == "org"
    with bob.store._connect() as db:
        rel = db.execute("SELECT origin FROM entity_relations WHERE entity_a='dana' "
