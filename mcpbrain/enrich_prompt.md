...
```json
{
  ...
  "messages": [{"message_id": "m-1",
                "sender": "Dana Okafor <dana@centrepoint.church>",
...
- `waiting_on` (on an action): optional. Set it to the name of the person the
  action is awaiting a reply or input from ("Josh is waiting on Marcus to confirm"
  -> `"waiting_on": "Marcus Reyes"`). Use the person's bare name, matching an
...
- **Naming.** An entity `name` is the bare proper name, nothing else. Strip
  ...
  becomes `Optus Stadium`, "Pastor Dana Okafor" becomes `Dana Okafor`.
...
```json
{"pair_id": "a-id|b-id", "same": true, "canonical": "Dana Okafor"}
```
