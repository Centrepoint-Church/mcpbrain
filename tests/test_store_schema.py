...
def test_store_email_context_writer(tmp_path):
    ...
    s.upsert_email_context(
        "m-1", subject="Hall B", sender="Dana <dana@x>", sender_email="dana@x",
...
def test_entities_dedup_columns_backfilled_on_old_store(tmp_path):
    ...
    db.execute("INSERT INTO entities(id,name,type) VALUES('dana','Dana','person')")
    db.commit()
    db.close()
    ...
    for col in ("aliases", "email_addr", "notes"):
        assert col in cols
    with s._connect() as db:
        row = db.execute("SELECT name FROM entities WHERE id='dana'").fetchone()
...
def test_entities_degree_email_count_backfilled_on_old_store(tmp_path):
    ...
    db.execute("""CREATE TABLE entities(
        ...
        org TEXT DEFAULT '', first_seen TEXT DEFAULT '',
        last_seen TEXT DEFAULT '', mentions INTEGER DEFAULT 0)""")
    db.execute("INSERT INTO entities(id,name,type,mentions) VALUES('dana','Dana','person',5)")
    db.commit()
    db.close()
    ...
    with s._connect() as db:
        row = db.execute("SELECT name, mentions, degree, email_count "
                         "FROM entities WHERE id='dana'").fetchone()
    assert row["name"] == "Dana"
...
def test_entity_relations_bitemporal_migration_preserves_data(tmp_path):
    ...
    with s._connect() as db:
        row = db.execute("SELECT entity_a, relation, entity_b, source_doc_id "
                         "FROM entity_relations WHERE entity_a='dana'").fetchone()
...
def test_origin_defaults_to_local_and_backfills_old_store(tmp_path):
    ...
    db.execute("INSERT INTO entity_relations(entity_a,relation,entity_b) "
               "VALUES('dana','works_at','acme')")
    db.commit(); db.close()

    s = Store(path, dim=4); s.init()
    with s._connect() as conn:
        assert "origin" in _cols(s, "entities")
        ent = conn.execute("SELECT origin FROM entities WHERE id='dana'").fetchone()
        rel = conn.execute("SELECT origin FROM entity_relations "
                           "WHERE entity_a='dana'").fetchone()
    ...
    with s._connect() as db:
        for _ in range(2):
            db.execute(
                ...
                ("a@x.org", "deadbeef", '{"kind":"entity","id":"dana"}'))
