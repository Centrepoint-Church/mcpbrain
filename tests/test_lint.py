...
def test_check_possible_duplicates_flags(tmp_path):
    """Two person entities 'Marcus Reyes' / 'Marcus H' same org -> flagged (score >= 75)."""
    s = _store(tmp_path)
    _add_entity(s, "marcus-reyes", "Marcus Reyes", org="Centrepoint", email_count=5)
    _add_entity(s, "marcus-h", "Marcus H", org="Centrepoint", email_count=2)
    ...
    assert "marcus-reyes" in all_ids or "marcus-h" in all_ids, (
        f"Marcus Reyes / Marcus H pair not found in: {flagged}"
    )
    # Verify score >= 75
    pair = next(r for r in flagged if "marcus-reyes" in (r["id_a"], r["id_b"]))
