...
def test_person_without_email_identity_is_not_contributed(tmp_path):
    ...
    s = _store(tmp_path)
    d = _delta(a_email="")                                # dana has no email
...
def test_delta_since_watermark_excludes_stale_already_seen_relation(tmp_path):
    ...
    with s._connect() as db:
        for eid, name, typ in (("dana", "Dana", "person"), ("acme", "Acme", "org")):
            db.execute("INSERT INTO entities(id,name,type,origin) VALUES(?,?,?,'local')",
                       (eid, name, typ))
    rid = graph_write.upsert_relation(s, "dana", "works_at", "acme", valid_from="2026-01-01",
                                      source_doc_id="msg-1")
    delta, wm = org_contrib._delta_since_watermark(s)
    assert wm["hwm"] == rid
    s.set_meta("org_contrib_hwm", str(wm["hwm"]))
...
def test_delta_since_watermark_picks_up_same_second_reobservation(tmp_path):
    ...
    assert ("dana", "works_at", "acme") in rels2
...
def test_delta_since_watermark_picks_up_same_second_supersession(tmp_path):
    ...
    the SAME wall-clock second as the persisted watermark checkpoint must still
    be picked up. works_at is a singleton relation, so upserting dana->beta
    supersedes the existing dana->acme row (graph_write._mark_superseded sets
    ...
    with s._connect() as db:
        for eid, name, typ in (("dana", "Dana", "person"), ("acme", "Acme", "org"),
            ...
            db.execute("INSERT INTO entities(id,name,type,origin) VALUES(?,?,?,'local')",
                       (eid, name, typ))
    old_rid = graph_write.upsert_relation(s, "dana", "works_at", "acme", valid_from="2026-01-01",
                                          source_doc_id="msg-1")
    delta, wm = org_contrib._delta_since_watermark(s)
    ...
    # Supersede dana->acme with dana->beta (newer valid_from -> the singleton
    # recency rule retires acme), then force invalidated_at to land EXACTLY on
    # the watermark second.
    graph_write.upsert_relation(s, "dana", "works_at", "beta", valid_from="2026-02-01",
                                source_doc_id="msg-2")
    with s._connect() as db:
    ...
    rels2 = {(r["entity_a"], r["relation"], r["entity_b"]) for r in delta2["relations"]}
    assert ("dana", "works_at", "beta") in rels2      # and the new current row too
...
def test_delta_since_watermark_hwm_reflects_fetched_rows_not_a_later_insert(tmp_path):
    ...
    with s._connect() as db:
        for eid, name, typ in (("dana", "Dana", "person"), ("acme", "Acme", "org"),
            ...
            db.execute("INSERT INTO entities(id,name,type,origin) VALUES(?,?,?,'local')",
                       (eid, name, typ))
    rid1 = graph_write.upsert_relation(s, "dana", "works_at", "acme", valid_from="2026-01-01",
                                       source_doc_id="msg-1")
    delta, wm = org_contrib._delta_since_watermark(s)
    assert wm["hwm"] == rid1
    s.set_meta("org_contrib_hwm", str(wm["hwm"]))
    ...
    rid2 = graph_write.upsert_relation(s, "dana", "mentioned_with", "beta", valid_from="2026-02-01",
                                       source_doc_id="msg-2")
    delta2, wm2 = org_contrib._delta_since_watermark(s)
    rels2 = {(r["entity_a"], r["relation"], r["entity_b"]) for r in delta2["relations"]}
    assert ("dana", "mentioned_with", "beta") in rels2
    assert wm2["hwm"] == rid2
...
def test_person_name_freetext_without_separator_is_dropped(tmp_path):
    ...
    s = _store(tmp_path)
    d = _delta()
    d["entities"]["dana"]["name"] = "Bob divorce lawyer"     # emailed person, junk name
