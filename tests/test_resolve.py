...
class _DanaClient:
    ...
    adjudication are Dana variants. The A:/B: lines carry the names being
    ...
    def generate_content(self, model=None, contents=None, config=None):
        ...
        names = " ".join(compared)
        if "Dana" in names and "Daniel" not in names:
            payload = {"same": True, "canonical": "Dana Okafor"}
        else:
...
def test_adjudicate_parses_same_true():
    out = _adjudicate(_DanaClient(), {"name": "Dana", "type": "person"},
                      {"name": "Dana Okafor", "type": "person"})
    assert out["same"] is True
    assert out["canonical"] == "Dana Okafor"
...
def test_resolve_live_adjudicates_fuzzy_correctly(tmp_path):
    ...
    out = resolve_entities(store, client=_DanaClient())
    ...
    ids = {e["id"] for e in store.list_entities()}
    # Dana pair merged into one survivor named "Dana Okafor".
    assert ("dana" in ids) ^ ("dana-okafor" in ids)
    survivor = next(e for e in store.list_entities()
                    if e["id"] in ("dana", "dana-okafor"))
    assert survivor["name"] == "Dana Okafor"
...
def test_resolve_api_error_does_not_crash(tmp_path):
    ...
    store.init()
    # Deterministic pair: same canonical key "dana". "dana" has more mentions,
    # so it survives and "ps-dana" folds into it.
    store.upsert_entity("dana", "Dana", "person", seen="2026-05-30")
    store.upsert_entity("dana", "Dana", "person", seen="2026-05-30")
    store.upsert_entity("ps-dana", "Ps Dana", "person", seen="2026-05-30")
    ...
    assert out["llm_calls"] == 0
    ids = {e["id"] for e in store.list_entities()}
    # Deterministic merge of Ps Dana into Dana still happened.
    assert "ps-dana" not in ids
    assert "dana" in ids
...
def test_resolve_idempotent_second_run(tmp_path):
    store = _seed_fuzzy_store(tmp_path)
    resolve_entities(store, client=_DanaClient())

    # Second run: nothing left to merge. Any remaining distinct pair (Daniel)
    # returns false, and the Dana pair is already one entity.
...
def test_adjudicate_escapes_double_quotes_in_name():
    ...
    a = {"name": 'Dana "JC" Okafor', "type": "person"}
    b = {"name": "Dana Okafor", "type": "person"}
    ...
    class _InspectingClient:
        ...
        def generate_content(self, model=None, contents=None, config=None):
            ...
            return _Resp(json.dumps({"same": True, "canonical": "Dana Okafor"}))
    ...
    assert '"Dana \\"JC\\" Okafor"' in client.last_prompt
