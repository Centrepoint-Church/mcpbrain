...
def test_backup_artifact_decrypts_to_a_valid_store(tmp_path):
    store = _store_with_chunk(tmp_path)
    store.upsert_entity("marcus-reyes", "Marcus Reyes", "person", org="Centrepoint")
...
def _seed_duplicate_entities(store):
    ...
    ('Ps Dana' and 'Dana' both -> 'dana', type person). The deterministic
    resolver folds them into one survivor; without resolution both remain."""
    store.upsert_entity("ps-dana", "Ps Dana", "person", org="Centrepoint")
    store.upsert_entity("dana", "Dana", "person", org="Centrepoint")
    return ["ps-dana", "dana"]
...
def test_resolve_not_due_skips_then_due_runs_again(tmp_path):
    ...
    store.upsert_entity("ps-marcus", "Ps Marcus", "person", org="Centrepoint")
    store.upsert_entity("marcus", "Marcus", "person", org="Centrepoint")
