...
def test_org_from_email_known_domains():
    assert gw.org_from_email("dana@centrepoint.church", _CP_TAXONOMY) == "Centrepoint"
...
def test_entity_slug():
    assert gw.entity_slug("Marcus Reyes") == "marcus-reyes"
...
def test_upsert_entity_email_dedup(tmp_path):
    ...
    first = gw.upsert_entity(s, name="Dana Okafor", entity_type="person",
                             org="Centrepoint", email_addr="dana@centrepoint.church")
    ...
    second = gw.upsert_entity(s, name="J. Okafor", entity_type="person",
                              email_addr="dana@centrepoint.church")
    assert second == first
...
def test_apply_writes_entities_and_email_context(tmp_path):
    ...
    # Dana entity exists (sender, from dana@centrepoint.church).
    ...
    assert ec["labels"] == ""
    # Sender linked.
    sender_links = [lk for lk in links if lk["entity_id"] == dana["id"]]
...
def test_apply_writes_waiting_on(tmp_path):
    ...
    ext["entities"] = ext["entities"] + [
        {"name": "Marcus Reyes", "type": "person", "org": "Centrepoint", "role": ""}]
    ext["actions"] = [{
...
def test_action_routed_with_owner_and_status(tmp_path):
    ...
    ext = _thread(actions=[_action("Prepare the venue checklist",
                  ...
                  lead_sender="Marcus Reyes <marcus@centrepoint.church>")
    gw.apply(s, ext, doc_ids=["d1"], clock=_clock)
    rows = s.list_unified_actions()
    assert len(rows) == 1
...
def test_apply_full_lifecycle_summary(tmp_path):
    ...
    ext = {
        ...
        "messages": [
            {"message_id": "m-rich", "sender": "Marcus Reyes <marcus@centrepoint.church>",
    ...
    assert marcus_act and josh_act
...
def test_strip_affiliation_strips_from_known_org():
    assert gw.strip_affiliation("Dana from Centrepoint") == "Dana"
...
def test_known_domain_sender_gets_its_org(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MCPBRAIN_HOME", str(tmp_path))
    s = _store(tmp_path)
    gw.apply(s, _sender_ext("ACC", "Dana <dana@centrepoint.church>"),
             doc_ids=["d1"])
    dana = s.find_entity("Dana")
    assert dana is not None
    assert dana["org"] == "Centrepoint"
