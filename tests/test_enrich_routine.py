...
def test_routine_is_queue_driven_and_capless():
    ...
    low = text.lower()
    # Terminator is the empty queue (pending == 0), not a wave/budget cap.
    assert "brain_enrich_pending" in text
    assert "empty" in low
    ...
    # Spawns a pool of Haiku drainers that self-serve via claim, and nudges the daemon.
    assert "enrich-batch" in text
    assert "brain_enrich_claim" in text
    assert "haiku" in low
