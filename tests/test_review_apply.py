...
def test_suppress_verdict_suppresses_entity_and_resolves_finding(tmp_path):
    ...
    assert result == {"suppressed": 1, "kept": 0, "skipped": 0, "capped": 0}
...
def test_keep_verdict_resolves_finding_without_suppressing(tmp_path):
    ...
    with s._connect() as db:
        row = db.execute("SELECT * FROM suppressed_entities WHERE entity_id='e2'").fetchone()
    assert row is None
    assert s.open_findings("lint:orphan_entity") == []
    assert result == {"suppressed": 0, "kept": 1, "skipped": 0, "capped": 0}
...
def test_unrecognised_verdict_treated_as_skip(tmp_path):
    ...
    with s._connect() as db:
        row = db.execute("SELECT * FROM suppressed_entities WHERE entity_id='e3'").fetchone()
    assert row is None
    assert s.open_findings("lint:orphan_entity") == []  # still resolved, just no mutation
    assert result == {"suppressed": 0, "kept": 0, "skipped": 1, "capped": 0}
...
def test_cap_stops_applying_suppressions(tmp_path):
    ...
    with s._connect() as db:
        rows = db.execute("SELECT entity_id FROM suppressed_entities").fetchall()
    assert len(rows) == 1
