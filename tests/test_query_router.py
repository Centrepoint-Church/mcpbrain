...
def test_classify_intent_entity_match():
    store = _make_store(entities={"dana": {"id": "dana-id", "name": "Dana"}})
    intent, eid = _classify_intent("Dana's decisions on the budget", store)
...
def test_route_routing_on_uses_graph_seed(tmp_path):
    ...
    store = _make_store(
        entities={"dana": {"id": "dana-id", "name": "Dana"}},
        relations={"dana-id": [{"entity_a": "dana-id", "entity_b": "bob-id"}]},
    )
    ...
    with patch("mcpbrain.query_router.hybrid_search", return_value=expected):
        result = route(store, embedder, "Dana decisions", 5, home=home)
