...
def test_ingest_skips_malformed_lines_but_ingests_rest(tmp_path):
    ...
    fs = LocalDirFleetStorage(tmp_path / "fleet")
    s = _store(tmp_path)
    good = _rec({"kind": "entity", "id": "dana", "name": "Dana", "type": "person",
                 "org": "", "email_addr": "", "aliases": ""})
...
def test_ingest_distinguishes_claims_with_same_source_ref(tmp_path):
    ...
    fs = LocalDirFleetStorage(tmp_path / "fleet")
    s = _store(tmp_path)
    r1 = _rec({"kind": "entity", "id": "dana", "name": "Dana", "type": "person",
               "org": "", "email_addr": "", "aliases": ""})
    r2 = _rec({"kind": "entity", "id": "dana", "name": "Dana Smith", "type": "person",
               "org": "", "email_addr": "", "aliases": ""})
...
def test_rematerialise_updates_org_skeleton_with_new_info(tmp_path):
    ...
    s = _store(tmp_path)
    _stage(s, [_rec({"kind": "entity", "id": "dana", "name": "Dana", "type": "person",
                     "org": "", "email_addr": "", "aliases": ""})])
    org_curate._materialise(s)
    dana = s.get_entity("dana")
    assert dana["origin"] == "org" and dana["email_addr"] == ""
    # A later contribution reports dana's email — re-materialising (the
    ...
    org_curate._materialise(s)
    dana = s.get_entity("dana")
    assert dana["origin"] == "org"
    assert dana["email_addr"] == "dana@acme.org" and dana["org"] == "Acme"
...
def test_relation_reasserted_as_ongoing_overrides_earlier_end_date(tmp_path):
    ...
    with s._connect() as db:
        row = db.execute("SELECT valid_to FROM entity_relations "
                         "WHERE entity_a='dana' AND relation='works_at' AND entity_b='acme'").fetchone()
...
def test_relation_supersession_not_clobbered_by_contributed_valid_to(tmp_path):
    ...
    triple (dana now works_at beta instead of acme), that supersession's own
    ...
    _stage(s, [
        ...
        _rec({"kind": "relation", "entity_a": "dana", "relation": "works_at", "entity_b": "beta"},
    ...
    with s._connect() as db:
        ...
        beta_rel = db.execute("SELECT valid_to FROM entity_relations "
                              "WHERE entity_a='dana' AND relation='works_at' AND entity_b='beta'").fetchone()
...
def test_run_applies_real_adjudication_verdict_end_to_end(tmp_path, monkeypatch):
    ...
    pair_id = "|".join(sorted(("dana", "dana-okafor")))
