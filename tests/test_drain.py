...
def _envelope(thread_id, **overrides):
    ...
    env = {
        ...
        "relations": [],
        "messages": [
            {"message_id": f"{thread_id}-m1", "sender": "Dana <dana@centrepoint.church>",
...
def test_drain_sanitizes_empty_relations_keeps_extraction(store, home):
    ...
    env = _envelope("t-rel", relations=[
        {"source_name": "Dana", "type": "works_with", "target_name": "Josh"},
...
def test_drain_applies_merge_answers(store, home):
    ...
    store.upsert_entity("dana-okafor", "Dana Okafor", "person", "Centrepoint", "2026-04-01")
    store.upsert_entity("dana-okafor", "Dana Okafor", "person", "Centrepoint", "2026-04-02")  # mentions=2
    store.upsert_entity("j-okafor", "J Okafor", "person", "Centrepoint", "2026-04-01")  # mentions=1
