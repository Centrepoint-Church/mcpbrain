...
def _envelope(thread_id, **overrides):
    ...
    env = {
        ...
        "relations": [],
        "messages": [
            {"message_id": f"{thread_id}-m1", "sender": "Dana <dana@example.org>",
...
def test_drain_sanitizes_empty_relations_keeps_extraction(store, home):
    ...
    env = _envelope("t-rel", relations=[
        {"source_name": "Dana", "type": "works_at", "target_name": "Sam"},
...
def test_drain_persists_enrich_payload_for_drive_docs_only(store, home):
    ...
    env = _envelope(thread_id, messages=[
        {"message_id": "msg-drive", "sender": "Dana <dana@example.org>",
         "date": "2026-04-18", "labels": "INBOX", "subject": "Subject"},
        {"message_id": "msg-mail", "sender": "Dana <dana@example.org>",
         "date": "2026-04-18", "labels": "INBOX", "subject": "Subject"},
    ])
