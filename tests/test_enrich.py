...
GOOD_JSON = {
    "entities": [{"name": "Marcus Reyes", "type": "person", "org": "Centrepoint"}],
    "relations": [{"from": "Marcus Reyes", "relation": "reports_to", "to": "Dana Okafor"}],
...
def test_extract_skip_absent_is_not_skipped():
    ...
    out = extract(client, "body", {})
    assert out["entities"][0]["name"] == "Marcus Reyes"
...
def test_enrich_document_lands_expected_rows(tmp_path, monkeypatch):
    ...
    # entities = 2 new rows: marcus-reyes (declared) + dana-okafor (relation stub).
    ...
    marcus = s.get_entity("marcus-reyes")
    assert marcus is not None
    assert marcus["org"] == "Centrepoint"
    assert marcus["type"] == "person"
    assert marcus["last_seen"] == "2026-05-30"

    rels = s.list_relations()
    assert len(rels) == 1
    r = rels[0]
    assert (r["entity_a"], r["relation"], r["entity_b"]) == (
        "marcus-reyes", "reports_to", "dana-okafor")
...
def test_enrich_document_counts_rows_written_not_extracted(tmp_path):
    ...
    payload = {
        "entities": [
            {"name": "Marcus Reyes", "type": "person", "org": "Centrepoint"},
...
def test_enrich_document_filters_junk_entities_and_clamps_org(tmp_path, monkeypatch):
    ...
    payload = {
        "entities": [
            ...
            {"name": "Dana Okafor", "type": "person", "org": "Centrepoint"},
            {"name": "Worship Night", "type": "topic", "org": "WORSHIP"},
        ],
        "relations": [
            # URL endpoint -> structural junk -> relation skipped
            {"from": "https://example.com/signup", "relation": "works_at", "to": "Dana Okafor"},
        ],
        "actions": [],
        "decisions": [],
    }
    ...
    # real person kept
    dana = s.get_entity("dana-okafor")
    assert dana is not None and dana["org"] == "Centrepoint"
...
def test_canonical_name_strips_honorifics():
    ...
    assert _canonical_name("Ps Dana") == "Dana"
    assert _canonical_name("Pastor Dana Okafor") == "Dana Okafor"
    assert _canonical_name("Dr. Sarah") == "Sarah"


def test_canonical_name_leaves_plain_names():
    from mcpbrain.enrich import _canonical_name
    assert _canonical_name("Dana Okafor") == "Dana Okafor"
...
def test_enrich_document_clamps_invalid_declared_types_to_topic(tmp_path):
    ...
    assert s.get_entity("dana-okafor")["type"] == "person"
...
def test_enrich_document_canonicalises_declared_entity_name(tmp_path):
    ...
    assert s.get_entity("ps-dana-okafor") is None
...
def test_enrich_document_canonicalises_relation_endpoints(tmp_path):
    ...
    payload = {
        ...
        "relations": [
            {"from": "Marcus Reyes", "relation": "reports_to", "to": "Pastor Dana Okafor"},
        ],
        "actions": [],
        "decisions": [],
    }
...
def test_relation_endpoint_year_bearing_org_not_dropped(tmp_path):
    ...
    payload = {
        ...
        "relations": [
            {"from": "Dana Okafor", "relation": "works_at", "to": "Centrepoint 2026"},
        ],
        "actions": [],
        "decisions": [],
    }
...
def test_relation_endpoint_url_junk_still_dropped(tmp_path):
    ...
    payload = {
        ...
        "relations": [
            {"from": "Dana Okafor", "relation": "works_at", "to": "https://example.com"},
        ],
        "actions": [],
        "decisions": [],
    }
...
def test_run_enrichment_live_writes_rows_and_sets_mode(tmp_path):
    ...
    # Doc 1 creates marcus-reyes + dana-okafor stub (2 entities) and 1 relation.
    # Doc 2 merges both entities (no new rows) and hits the duplicate relation triple.
    assert result["entities"] == 2   # marcus-reyes + dana-okafor stub, created once
...
def test_run_enrichment_live_counts_match_written_rows(tmp_path):
    ...
    assert result["mode"] == "live"
    # 2 new entity rows: marcus-reyes (declared) + dana-okafor (relation stub).
...
def test_run_enrichment_dedup_counts_shared_rows_once(tmp_path):
    ...
    assert result["mode"] == "live"
    # marcus-reyes (declared) + dana-okafor (stub) created on doc-a only.
    ...
    assert s.get_entity("marcus-reyes")["mentions"] == 2
...
def test_run_enrichment_skip_marks_done_without_graph_rows(tmp_path):
    ...
    skip_payload = {"skip": True, "entities": [{"name": "Marcus", "type": "person"}],
                    "relations": [], "actions": [], "decisions": []}
