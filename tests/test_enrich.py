...
GOOD_JSON = {
    "entities": [{"name": "Marcus Reyes", "type": "person", "org": "Acme"}],
...
def test_enrich_document_filters_junk_entities_and_clamps_org(tmp_path, monkeypatch):
    ...
    # real person kept
    dana = s.get_entity("dana-okafor")
    assert dana is not None and dana["org"] == "Acme"
...
def test_enrich_document_canonicalises_declared_entity_name(tmp_path):
    ...
    payload = {
        "entities": [
            {"name": "Ps Dana Okafor", "type": "person", "org": "Acme"},
        ],
        "relations": [],
        "actions": [],
        "decisions": [],
...
def test_relation_endpoint_year_bearing_org_not_dropped(tmp_path):
    ...
    payload = {
        ...
        "relations": [
            {"from": "Dana Okafor", "relation": "works_at", "to": "Acme 2026"},
        ],
        "actions": [],
        "decisions": [],
    }
