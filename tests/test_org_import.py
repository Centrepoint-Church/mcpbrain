...
def test_malformed_relation_aborts(tmp_path):
    ...
    fs = LocalDirFleetStorage(tmp_path / "fleet")
    s = _store(tmp_path)
    _publish(fs, [_ent("dana", "Dana")],
             [{"entity_a": "dana", "relation": "", "entity_b": "acme"}], version=1)  # blank relation
    res = org_import.import_snapshot(s, fs)
...
def test_upsert_never_clobbers_local_relation_at_colliding_triple(tmp_path):
    ...
    s = _store(tmp_path)
    with s._connect() as db:
        db.execute("INSERT INTO entities(id,name,type,origin) VALUES('dana','Dana','person','local')")
        ...
        db.execute("INSERT INTO entity_relations(entity_a,relation,entity_b,valid_from,origin) "
                   "VALUES('dana','works_at','acme','2020-01-01','local')")
    _publish(fs, [_ent("dana", "Dana"), _ent("acme", "Acme", "org")],
             [{"entity_a": "dana", "relation": "works_at", "entity_b": "acme",
               "valid_from": "2026-01-01", "valid_to": "", "confidence": 1.0}], version=1)
    org_import.import_snapshot(s, fs)
...
def test_wholesale_replace_removes_absent_org_relation(tmp_path):
    ...
    fs = LocalDirFleetStorage(tmp_path / "fleet")
    s = _store(tmp_path)
    _publish(fs, [_ent("dana", "Dana"), _ent("acme", "Acme", "org"), _ent("beta", "Beta", "org")],
             [{"entity_a": "dana", "relation": "works_at", "entity_b": "acme",
               "valid_from": "2026-01-01", "valid_to": "", "confidence": 1.0}], version=1)
    org_import.import_snapshot(s, fs)
    with s._connect() as db:
        assert db.execute("SELECT COUNT(*) c FROM entity_relations WHERE origin='org'").fetchone()["c"] == 1
    # v2: dana now works_at beta instead — the dana->acme triple must be gone,
    # not left behind as a stale relation alongside the new one.
    _publish(fs, [_ent("dana", "Dana"), _ent("acme", "Acme", "org"), _ent("beta", "Beta", "org")],
             [{"entity_a": "dana", "relation": "works_at", "entity_b": "beta",
    ...
    assert rows == {("dana", "works_at", "beta")}
...
def test_fan_in_two_org_entities_matching_same_local_id_is_ambiguous(tmp_path):
    ...
    org_import.import_snapshot(s, fs)
    assert s.get_entity("dana-local") is not None    # neither org entity claimed it
    with s._connect() as db:
...
def test_restore_precision_survives_same_day_post_merge_observation(tmp_path):
    ...
    with s._connect() as db:                         # pre-merge flesh, dated today too
        db.execute("INSERT INTO entity_observations(entity_id,attribute,value,source,valid_from) "
                   "VALUES('dana-old','note','pre-merge','local',?)", (today,))
    _publish(fs, [_ent("dana-generic", "Dana Generic")], [], version=2,
             tombstones=[Tombstone(entity_id="dana-old", merged_into="dana-generic")])
    ...
    with s._connect() as db:                         # native observation added the SAME day, after the merge
        db.execute("INSERT INTO entity_observations(entity_id,attribute,value,source,valid_from) "
                   "VALUES('dana-generic','note','native-same-day','local',?)", (today,))
    _publish(fs, [_ent("dana-old", "Dana Old"), _ent("dana-generic", "Dana Generic")], [], version=3)
    org_import.import_snapshot(s, fs)                # curator splits dana-old back out
    with s._connect() as db:
        old_notes = {r["value"] for r in db.execute(
    ...
    assert generic_notes == {"native-same-day"}      # not pulled onto dana-old despite matching dates
