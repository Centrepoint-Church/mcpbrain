...
def test_format_relative_floor_trims_weak_tail():
    ...
    block, used = pr._format_context(hits, seen=set())
...
def test_format_dedups_against_seen():
    ...
    block, used = pr._format_context(hits, seen={"d1"})
...
def test_seen_file_prunes_stale_siblings(tmp_path):
    ...
    seen, path = pr._load_seen(str(tmp_path), "sNew", now=pr._SEEN_TTL_S + 10_000)
