...
def _seed_graph(s):
    s.upsert_entity("marcus-reyes", "Marcus Reyes", "person", org="Centrepoint")
    s.upsert_entity("dana-okafor", "Dana Okafor", "person", org="Centrepoint")
    ...
    s.add_action("Confirm college timetable", owner="Marcus Reyes")
...
def test_actions_for_owner_case_insensitive(tmp_path):
    s = _store(tmp_path)
    _seed_graph(s)
    acts = s.actions_for_owner("marcus reyes")
    assert len(acts) == 1
...
def test_merge_repoints_dedups_drops_self_loops_and_removes_loser(tmp_path):
    ...
    s.upsert_entity("ps-dana", "Ps Dana", "person", org="Centrepoint", seen="2026-05-30")
    ...
    assert all(r["entity_a"] != "ps-dana" and r["entity_b"] != "ps-dana" for r in rels)
    # Exactly one (dana, works_at, acc) — the collision was deduped.
    works = [r for r in rels if r["relation"] == "works_at"]
    assert len(works) == 1
    assert works[0]["entity_a"] == "dana" and works[0]["entity_b"] == "acc"
    # No self-loop.
...
def test_merge_scalar_precedence_and_mention_sum(tmp_path):
    ...
    s.upsert_entity("dana", "dana", "unknown", org="", seen="2026-05-30")  # mentions=1
    s.upsert_entity("ps-dana", "Ps Dana", "person", org="Centrepoint", seen="2026-05-30")
    s.upsert_entity("ps-dana", "Ps Dana", "person", org="Centrepoint", seen="2026-05-31")  # mentions=2
...
def test_merge_writes_audit_row(tmp_path):
    s = _store(tmp_path)
    s.upsert_entity("dana", "dana", "person", org="Centrepoint", seen="2026-05-30")
    s.upsert_entity("ps-dana", "Ps Dana", "person", org="Centrepoint", seen="2026-05-30")

    s.merge_entities("ps-dana", "dana", method="llm")

    merges = s.list_entity_merges()
    assert len(merges) == 1
    row = merges[-1]
    assert row["loser_id"] == "ps-dana"
    assert row["winner_id"] == "dana"
    assert row["method"] == "llm"
    assert row["loser_name"] == "Ps Dana"
...
def test_merge_is_noop_when_loser_equals_winner(tmp_path):
    s = _store(tmp_path)
    s.upsert_entity("dana", "Dana", "person", org="Centrepoint", seen="2026-05-30")
    s.merge_entities("dana", "dana")
    assert {e["id"] for e in s.list_entities()} == {"dana"}
...
def test_merge_is_noop_when_an_id_is_missing(tmp_path):
    ...
    s.merge_entities("ghost", "dana")
    s.merge_entities("dana", "ghost")
    assert {e["id"] for e in s.list_entities()} == {"dana"}
    assert s.list_entity_merges() == []
...
def test_entities_for_resolution_returns_five_fields(tmp_path):
    ...
    assert by_id["dana"]["name"] == "Dana"
    assert by_id["dana"]["org"] == "Centrepoint"
