#!/usr/bin/env bash
# mcpbrain installer (Linux). Installs uv if missing, installs the mcpbrain
# tool, warms the embedding model, registers the login agent, and opens the
# setup wizard. Pass --dry-run to print the steps without running them.
set -euo pipefail

# The installer lives in install/. Move up to the repo root so
# `uv tool install --from .` finds pyproject.toml regardless of the caller's CWD.
cd "$(dirname "${BASH_SOURCE[0]}")/.."
REPO="$(pwd)"

# Canonical home for register, daemon --once, and the agent. Matches the
# macOS/Windows installers so every step uses one directory.
...
# `mcpbrain setup` installs and starts the login agent itself (via
